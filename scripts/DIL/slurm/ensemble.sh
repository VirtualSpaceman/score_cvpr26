#!/bin/bash
#SBATCH --job-name=domain_ensemble                  # A name for your job
#SBATCH --output=slurm_logs/dil-ensemble/%x_%A_%a.out    # Standard output log: jobname_jobID_taskID.out
#SBATCH --error=slurm_logs/dil-ensemble/%x_%A_%a.err     # Standard error log
...
#SBATCH --mem=100G                           # Request 128GB of memory per job
...
# --- 3. Preparation ---
set -e
mkdir -p slurm_logs/dil-ensemble
...
python3 ensemble_domain_splitted_leave_one_out.py \
    --model "${model}" \
    --seed 5 \
    --dataset "${dataset}" \
