#!/bin/bash
#SBATCH --job-name=domain_finetune_ime          # A name for your job
#SBATCH --output=slurm_logs/dil-ft-time/%x_%A_%a.out    # Standard output log: jobname_jobID_taskID.out
#SBATCH --error=slurm_logs/dil-f-time/%x_%A_%a.err     # Standard error log
#SBATCH --partition=h100                    # The partition to run on (please change if needed)
#SBATCH --gres=gpu:1                        # Request 1 GPU per job
#SBATCH --cpus-per-task=16                  # Request 16 CPU cores per job
#SBATCH --mem=32G                           # Request 128GB of memory per job
#SBATCH --time=48:00:00                     # Time limit for each job (HH:MM:SS)
#SBATCH --array=1-24
...
# MODELS=("ViT-L-14")
DATASETS=("RetinaDomains" "PACS" "OfficeHome" "DomainNet" "NICOpp" "ImageNetR" "FedISIC" "TerraIncognita")
...
# --- 3. Preparation ---
set -e
mkdir -p slurm_logs/dil-ft-time
...
python3 finetune_domain_time.py \
    --model "${model}" \
    --seed 5 \
    --dataset "${dataset}" \
