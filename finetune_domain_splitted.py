...
def finetune(args, eval_0shot=False, only_eval_0shot=False):
    ...
    if not args.skip_eval:
        ...
        if eval_0shot or only_eval_0shot:
            ...
            for domain in dataset_class.DOMAINS:
                ...
                _r = eval_given_dataset(image_encoder, _dataset, train_dataset_name, args)
                _r = {k: v*100 for (k, v) in _r.items()}
                print({f'{domain}_metrics': _r })
    ...
    # EVAL FINETUNED MODEL ON EACH DOMAIN #
    # for domain in dataset_class.DOMAINS:
    #     _subset_config = {
    #         'domains': [domain],
    #         'classes': dataset_class.CLASSES
...
if __name__ == '__main__':
    ...
    for task_idx, domain_idx in enumerate(dataset_class.default_domain_order):
        ...
        print('='*100)
        print(f'Finetuning {args.model} on {args.dataset} - domain: {domain_str}({args.epochs} epoch(s))')
        print('='*100)
