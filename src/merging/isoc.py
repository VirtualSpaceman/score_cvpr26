...
def iso_cts(task_vectors, config):
    ...
    for key in task_vectors[0].vector:
        ...
        _task_specific_total_space_index_s = round((min(shape_) - common_space_index_s) / NUM_DATASETS) * NUM_DATASETS
        ...
        n_dims_per_task = int((min(shape_) - common_space_index_s) / NUM_DATASETS)
        for i, task_vector in enumerate(task_vectors):
        ...
        combined_space_u[:, NUM_DATASETS * n_dims_per_task : NUM_DATASETS * n_dims_per_task + common_space_index_s] = common_space_u
        combined_space_s[NUM_DATASETS * n_dims_per_task : NUM_DATASETS * n_dims_per_task + common_space_index_s] = common_space_s
        combined_space_v[NUM_DATASETS * n_dims_per_task : NUM_DATASETS * n_dims_per_task + common_space_index_s, :] = common_space_v
        
        ### Orthogonalize combined_space_u and combined_space_v ###
        u_combined_space_u, _, v_combined_space_u = torch.linalg.svd(combined_space_u, full_matrices=False)
        u_combined_space_v, _, v_combined_space_v = torch.linalg.svd(combined_space_v, full_matrices=False)
