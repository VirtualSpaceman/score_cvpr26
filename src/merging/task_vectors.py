...
class TaskVector():
    ...
    def __mul__(self, other):
        with torch.no_grad():
            ...
            for key in self.vector:
                new_vector[key] = self.vector[key] * other
                new_vector[key] = new_vector[key].to('cpu')
        return TaskVector(vector=new_vector)

    def dot(self, other, skip_layers=[]):
        ...
        with torch.no_grad():
            ...
            for key in self.vector:
                ...
                if len(skip_layers) > 0 and any([x in key for x in skip_layers]):
                    print(f"Skipping layer: {key} from dot product...")
                    continue 
                dot_product += torch.sum(self.vector[key].to(other.vector[key].device) * other.vector[key])
        return dot_product
...
def merge_rnd_mix(task_vectors):
    ...
    with torch.no_grad():
        ...
        for key in task_vectors[0].vector:
            ...
            _rand_indices = _rand_indices.to(task_vectors[0].vector[key].device)
            new_vector[key] = task_vectors[0].vector[key] * (_rand_indices == 0)
            for i in range(1, len(task_vectors)):
                new_vector[key] += task_vectors[i].vector[key].to(new_vector[key].device) * (_rand_indices == i)
